...
void Manacher(const string &str, int &pos, int &max_len) {
    ...
    // 本应该计算到expa_len + 1的，但那个位置是'#'，不需要计算了
    for (int max_R = 0, mid = 0, i = 2; i < expa_len; ++i) {
        // 根据i探查到的位置是否超过了最右回文子串的右边界
        // 初始化i的回文半径，核心操作
        if i < max_R {
            radius[i] = min(max_R-i, radius[2*mid-i]);
        } else {
            radius[i] = 1;
        }
        // radius[i] = i < max_R ? min(max_R-i, radius[2*mid-i]) : 1;
        ...
        if (radius[i] - 1 > max_len) {
            ...
            max_len = radius[i] - 1;
            // 原串的最长回文子串的起点
            pos     = (mid - radius[i] + 1) >> 1;
        }
    }
}

// odd 为 false，字符串为奇回文串
// 以 x 为中心的最长回文子串的长度
